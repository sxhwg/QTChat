...
ContactUserList::ContactUserList(QWidget *parent)
    : QListWidget(parent)
    , m_add_friend_item(nullptr), m_load_pending(false)
{
    Q_UNUSED(parent);
...
void ContactUserList::slot_add_auth_firend(std::shared_ptr<AuthInfo> auth_info)
{
    ...
    bool isFriend = UserMgr::GetInstance()->CheckFriendById(auth_info->m_uid);
    if(isFriend){
        return;
    }
    // 在 groupitem 之后插入新项
...
void ContactUserList::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    bool isFriend = UserMgr::GetInstance()->CheckFriendById(auth_rsp->m_uid);
    if(isFriend){
        return;
    }
    // 在 groupitem 之后插入新项
